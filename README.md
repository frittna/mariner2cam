...
**Status:** Work-in-progress, but working stable and fine now. I went through all the steps in my guide again from a fresh install.

> ⚠️ **Chitobox-Note:** The code from amd989 had to be modified a bit to work on new `.ctb` files (was a bug or caused by a Chitobox update).
...
The full step-by-step guide (1141 lines) including all configuration blocks and React frontend files (`Index.tsx` / `Files.tsx`) can be found here as an unformatted, secure text file:

➡️ **[Click here to view: CompleteInstructionZero2.txt](CompleteInstructionZero2.txt)**

*(You can download the file directly as clean text on the next page using the "Raw" button or the download icon in the top right corner).*
